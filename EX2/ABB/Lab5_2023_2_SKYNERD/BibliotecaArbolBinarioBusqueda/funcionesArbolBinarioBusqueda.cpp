    ...
    if(esNodoVacio(raiz))
        ...
        if(raiz->elemento.id > elemento.id)
            ...
            if(raiz->elemento.id < elemento.id)
                ...
                cout << "El elemento " << elemento.id << "Ya se encuentra en el arbol" << endl;
}
...
bool buscarRecursivo(struct NodoArbolBinarioBusqueda * nodo, const struct ElementoArbolBinarioBusqueda & elemento){
    if(esNodoVacio(nodo))
        return false;
    if(comparaElementos(nodo->elemento.id, elemento.id) == 0)
        return true;
    if(comparaElementos(nodo->elemento.id, elemento.id) == 1)
...
struct NodoArbolBinarioBusqueda * eliminarRecursivo(struct NodoArbolBinarioBusqueda * nodo,
    ...
    if(nodo->elemento.id > elemento.id)
