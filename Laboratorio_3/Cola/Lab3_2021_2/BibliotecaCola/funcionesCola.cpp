...
void imprimir_reves(const struct Cola &cola, struct NodoCola* nodo) {

    if (esColaVacia (cola)) cout<<"Vacio"<<endl;
    if (nodo==nullptr) return;
    imprimir_reves(cola, nodo->siguiente);
    cout<<nodo->elemento.piso<<"-"<<nodo->elemento.movimiento<<" -> ";
}